 ...
 * 会話の始まりも、途中と同じ形で渡す。
 *
 * 切り出す役目を 1 発言目に負わせると、そこだけ別の形が出る。
 * 話題の復唱、「頭から洗う派か、髪からか。」のような自問、
 * 「金欠ネタで一言。」のような段取りの宣言。
 * 禁じる形を並べても、次の形が出るだけで終わらなかった。
 *
 * そこでお題を司会の発言として履歴の先頭に置く。
 * 1 発言目も「相手の発言を受けて返す」形になり、切り出す役目が消える。
 *
 * これで減ったが、消えてはいない。10 回に 1 回ほど残る。
 */
function buildContext(topic: string, history: Line[]): string {
  return [
    ...
    "",
    "あなたの次の発言を返す。",
    "答えそのものから書く。話題を言い直さない。これから何を話すかも言わない。",
